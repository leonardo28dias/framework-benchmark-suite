...
1. Declaration	define_collection.py	Compiles raw text filter queries into structured condition mappings via an AST parser.	

In: CLI Filter Strings

Out: collections/<id>.yaml
2. Ingestion	download_collection.py	Converts bounds into remote API queries, profiles active memory streams, and enforces license/citation lookups.	

In: YAML Blueprint

Out: data/raw/<collection>/data.csv
3. Cataloging	build_catalog.py	Conducts structural diagnostics, infers machine learning tasks, and caches provenance variables.	

In: Raw Data CSVs

Out: catalog/catalog.csv & YAMLs
4. Sanitization	validate_datasets.py	Executes physical row/column drops, enforces sizing limits, and writes finalized evaluation subsets.	

In: Central Catalog Ledger

Out: data/processed/ & .resolved.yaml
── Quick Start ──

The pipeline scripts are completely interconnected via python import hooks. Executing the initial download step will automatically trigger a downstream chain: download_collection.py ⟶ build_catalog.py ⟶ validate_datasets.py.
1. Define a Target Collection Specification

Generate a declarative requirement profile by passing text expressions. Relational parameters are parsed using an Abstract Syntax Tree (AST) compiler engine:
Bash
...
2. Run the Automated Ingestion and Cleaning Pipeline

Execute the download utility against your generated file profile. The engine streams tables into memory, enforces strict legal/academic link validation, saves raw data, builds the metadata catalog, and exports the clean data matrices:
Bash

python scripts/download_collection.py --collection binary_clean_core

3. Isolated Component Manual Overrides

If you need to execute specific lifecycle stages manually over modified files, use the localized module entry points:
Bash
...
1. Core Clean Binary Classification Suite
...
2. Core Imbalanced Multiclass Suite
Dataset ID	Instances	Features	Accuracy	Macro Precision	Macro Recall	Macro F1	ROC AUC
...
3. Core Standard Regression Suite
